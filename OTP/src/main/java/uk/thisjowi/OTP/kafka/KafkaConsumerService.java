    ...
    public void handleUserRegisteredEvent(String message) {
        try {
            ...
            if ("USER_REGISTERED".equals(event.getEventType())) {
                logger.info("Processing user registration event for user: {}", event.getUsername());
                
                // Automatically create an OTP for the newly registered user
                // Valid for 30 days (2592000 seconds)
                otpService.createOtpForUser(event.getUserId(), event.getUsername(), "TOTP", 2592000L);
                
                logger.info("OTP automatically created for user: {}", event.getUsername());
            }
